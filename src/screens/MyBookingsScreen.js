...
import {
  ...
  Image, SafeAreaView, StatusBar, RefreshControl, Alert,
  ActivityIndicator,
} from 'react-native';
...
function BookingCard({ booking, onCancel, onPress }) {
  ...
  return (
    <TouchableOpacity style={styles.card} activeOpacity={0.9} onPress={() => onPress(booking)}>
      <View style={styles.cardTop}>
        <Image
          source={{ uri: hotel.image || 'https://images.unsplash.com/photo-1542314831-068cd1dbfeeb?w=400' }}
          style={styles.hotelImg}
          resizeMode="cover"
        />
        <View style={styles.cardInfo}>
          <Text style={styles.hotelName} numberOfLines={1}>{hotel.name || 'Hotel'}</Text>
          <Text style={styles.hotelLoc} numberOfLines={1}>📍 {hotel.location || ''}</Text>
          <View style={[styles.statusBadge, { backgroundColor: config.bg }]}>
            <Text style={[styles.statusText, { color: config.color }]}>● {config.label}</Text>
          </View>
        </View>
        <Text style={styles.bookingIdSmall}>#{booking._id?.slice(-6).toUpperCase()}</Text>
      </View>
      ...
      <View style={styles.cardBottom}>
        <View>
          <Text style={styles.guestsText}>{booking.guests} Guest{booking.guests !== 1 ? 's' : ''} · {booking.rooms} Room{booking.rooms !== 1 ? 's' : ''}</Text>
          <Text style={styles.totalText}>₹{booking.totalAmount?.toLocaleString()}</Text>
        </View>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <BookingCard
              ...
              onPress={(b) => navigation.navigate('BookingDetail', { bookingId: b._id, booking: b })}
            />
          )}
          ListEmptyComponent={
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptyText}>Your confirmed bookings will appear here.</Text>
              <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  filterTab: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20, backgroundColor: WHITE, borderWidth: 1.5, borderColor: '#E8E0D5' },
  ...
  card:       { backgroundColor: WHITE, borderRadius: 18, marginBottom: 14, overflow: 'hidden', elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.07, shadowRadius: 8 },
  ...
  statusBadge:{ alignSelf: 'flex-start', borderRadius: 8, paddingHorizontal: 10, paddingVertical: 4, marginTop: 4 },
  ...
  datesRow:    { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 14 },
  dateBlock:   {},
  dateLabel:   { fontSize: 10, color: MUTED, fontWeight: '700', letterSpacing: 0.6, marginBottom: 4 },
  dateVal:     { fontSize: 13, fontWeight: '700', color: DARK },
  nightsChip:  { alignItems: 'center', backgroundColor: CREAM, borderRadius: 10, paddingHorizontal: 12, paddingVertical: 6 },
  ...
  cancelBtn:   { borderWidth: 1.5, borderColor: RED, borderRadius: 10, paddingHorizontal: 18, paddingVertical: 8 },
  ...
  emptyText: { fontSize: 14, color: MUTED, textAlign: 'center', marginBottom: 24, lineHeight: 20 },
  browseBtn: { backgroundColor: DARK, borderRadius: 14, paddingHorizontal: 28, paddingVertical: 14 },
