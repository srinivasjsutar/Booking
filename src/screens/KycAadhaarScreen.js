...
export default function KycAadhaarScreen({ navigation }) {
  ...
  const handleSendOtp = async () => {
    ...
    if (raw.length !== 12) return Alert.alert('Invalid', 'Enter a valid 12-digit Aadhaar number.');
    setLoading(true);
    try {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      if (res.success) {
        navigation.replace('KycPan', { aadhaarName: res.name, aadhaarDob: res.dob });
      } else {
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.stepRow}>
          <View style={styles.stepActive}><Text style={styles.stepTxt}>1</Text></View>
          <View style={styles.stepLine} />
          <View style={styles.stepInactive}><Text style={styles.stepTxtGray}>2</Text></View>
        </View>

        <Text style={styles.tag}>Step 1 of 2</Text>
        <Text style={styles.title}>Aadhaar Verification</Text>
        <Text style={styles.sub}>OTP will be sent to your Aadhaar-linked mobile</Text>
        ...
        <View style={styles.card}>
          {step === 'entry' ? (
            <>
              ...
              />
              <Text style={styles.hint}>🔒 Encrypted & never stored in plain text</Text>
              <TouchableOpacity style={[styles.btn, loading && styles.btnOff]} onPress={handleSendOtp} disabled={loading}>
            ...
            <>
              ...
              <Text style={styles.sub2}>Sent to mobile linked with Aadhaar ••{aadhaar.replace(/\s/g,'').slice(-2)}</Text>
              <TextInput
              ...
              <TouchableOpacity style={[styles.btn, loading && styles.btnOff]} onPress={handleVerifyOtp} disabled={loading}>
                {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.btnTxt}>Verify OTP</Text>}
              </TouchableOpacity>
              <TouchableOpacity onPress={() => { setStep('entry'); setOtp(''); }} style={{ marginTop: 12 }}>
...
const styles = StyleSheet.create({
  ...
  stepInactive: { width: 32, height: 32, borderRadius: 16, backgroundColor: '#E5E7EB', alignItems: 'center', justifyContent: 'center' },
  ...
  input: { borderWidth: 1.5, borderColor: '#D1D5DB', borderRadius: 12, paddingHorizontal: 16, paddingVertical: 13, fontSize: 18, color: '#111827', backgroundColor: '#F9FAFB', marginBottom: 8 },
  hint:  { fontSize: 11, color: '#9CA3AF', marginBottom: 20 },
  btn:   { backgroundColor: BRAND, borderRadius: 14, paddingVertical: 15, alignItems: 'center', marginTop: 8 },
  btnOff: { opacity: 0.6 },
  btnTxt: { color: '#fff', fontSize: 16, fontWeight: '700' },
  resend: { color: BRAND, fontWeight: '600', fontSize: 13, textAlign: 'center', marginBottom: 16 },
